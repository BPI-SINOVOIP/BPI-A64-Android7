...
static int ieee80211_change_mac(struct net_device *dev, void *addr)
{
	...
	if (ret == 0){
		/*set the mac to vif addr */
 		memcpy(sdata->vif.addr, sa->sa_data, ETH_ALEN);
	/*add by gyq  the frist mac address is for p2p0, requiring the p2p0 's mac address change into wlan0 mac address*/
	/*p2p0 mac address  bit0 should be 0 ,bit1 should be 1,wlan0 mac bit0,bit1 should be 0,it decide by wpa_supplicant*/
	if(sdata->vif.addr[0]&0x02){

		memcpy(sdata->local->hw.wiphy->perm_addr, sdata->vif.addr, ETH_ALEN);
		sdata->local->hw.wiphy->perm_addr[0] = sdata->local->hw.wiphy->perm_addr[0]&0xFD;
