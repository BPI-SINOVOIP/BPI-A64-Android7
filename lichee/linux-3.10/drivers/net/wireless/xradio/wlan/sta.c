...
/*adb by gyq*/
#ifdef CONFIG_XRADIO_SET_MAC
	memcpy(hw_priv->mac_addr, vif->addr, ETH_ALEN);
	/*judge the mac address whether the p2p0 mac address,then set the p2p0 mac address*/
	if(vif->addr[0]&0x02){
		/*set p2p0 mac address*/
		memcpy(hw_priv->addresses[1].addr, vif->addr, ETH_ALEN);
	}else{
		/*set the wlan0 mac address*/
		memcpy(hw_priv->addresses[0].addr, vif->addr, ETH_ALEN);
		memcpy(hw_priv->mac_addr, hw_priv->addresses[0].addr, ETH_ALEN);
	}
	xradio_setup_mac(hw_priv);
#endif
