...
static int scaler_resource_alloc(struct scaler_dev *scaler)
{
	...
	os_mem_alloc(&scaler->vipp_reg);
	os_mem_alloc(&scaler->osd_para);
	os_mem_alloc(&scaler->osd_stat);

	if (scaler->vipp_reg.phy_addr != NULL) {
		if (!scaler->vipp_reg.vir_addr) {
			vin_err("scaler regs load vaddr requset failed!\n");
			return -ENOMEM;
		}
	} else {
		vin_err("scaler regs load paddr requset failed!\n");
		return -ENOMEM;
	}

	if (scaler->osd_para.phy_addr != NULL) {
		if (!scaler->osd_para.vir_addr) {
			vin_err("scaler para load vaddr requset failed!\n");
			return -ENOMEM;
		}
	} else {
		vin_err("scaler para load paddr requset failed!\n");
		return -ENOMEM;
	}

	if (scaler->osd_stat.phy_addr != NULL) {
		if (!scaler->osd_stat.vir_addr) {
			vin_err("scaler statistic load vaddr requset failed!\n");
			return -ENOMEM;
		}
	} else {
		vin_err("scaler statistic load paddr requset failed!\n");
		return -ENOMEM;
	}
	return 0;
...
static void scaler_resource_free(struct scaler_dev *scaler)
{
	...
	os_mem_free(&scaler->osd_para);
	os_mem_free(&scaler->osd_stat);
}
