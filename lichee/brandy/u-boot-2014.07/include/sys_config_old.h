...
typedef struct
{
	...
	int (*parser_offset)(char *script_mod_buf, ulong script_main_key_count,char *main_name);
	int (*parser_fetch_by_offset)(char * script_mod_buf,ulong script_main_key_count,int offset, char *sub_name, uint32_t value[]);
