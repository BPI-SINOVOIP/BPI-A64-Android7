...
int sunxi_oem_op_lock(int request_flag, char *info, int force)
{
	...
	if(request_flag == SUNXI_LOCKED)	//要求加锁
	{
		if( (store_flag == SUNXI_UNLOCKED) || (force == 1) )	//如果没有锁上
		{
			if(!signature_erase_all(mbr))//开始擦除数据
			{
				if(!force)
				{
					mbr->lockflag = SUNXI_LOCKED;
				}
				else
				{
					mbr->lockflag = SUNXI_LOCKED;
	...
	else if(request_flag == SUNXI_UNLOCKED)	//要求解锁
	{
		if( (store_flag == SUNXI_UNLOCKED) && (force != 1) )//
		{
...
int signature_erase_all(sunxi_mbr_t  *mbr)
{
	...
	for(i=0;i<mbr->PartCount;i++)
	{
        if (!strcmp("UDISK", (const char *)mbr->array[i].name))
		{
