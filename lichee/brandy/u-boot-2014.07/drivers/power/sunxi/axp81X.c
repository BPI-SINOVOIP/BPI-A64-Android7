...
int axp81_probe(void)
{
	...
	if(pmu_type == 0x41)
	{
		...
		//read TS
		if(axp_i2c_read(AXP81X_ADDR, 0x84, &pmu_type))
		{
			printf("axp read error\n");
			return -1;
		}
		tick_printf("PMU: read AXP81X 84H=0x%x\n",pmu_type);
		pmu_type &= 0xf8;
		pmu_type |= 0x06;
		if(axp_i2c_write(AXP81X_ADDR, 0x84, pmu_type))
		{
			printf("axp read error\n");
			return -1;
		}
		tick_printf("PMU: write AXP81X 84H=0x%x\n",pmu_type);
		return 0;
	}
