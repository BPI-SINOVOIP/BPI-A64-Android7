...
//	file_size = (file_size + 1023) & (~(1024 - 1));
	...
	fseek(img_file_hd, (uint)offset, SEEK_SET);
...
uint Img_WriteItem(HIMAGE hImage, HIMAGEITEM hItem, void *buffer, uint size)
{
	IMAGE_HANDLE* pImage = (IMAGE_HANDLE *)hImage;
	ITEM_HANDLE * pItem  = (ITEM_HANDLE  *)hItem;
	long long	  offset;
...
//	file_size = (file_size + 1023) & (~(1024 - 1));
	...
	offset |= pImage->ItemTable[pItem->index].offsetLo;
	fseek(img_file_hd, (uint)offset, SEEK_SET);
	if(!fwrite(buffer, file_size, 1, img_file_hd))
	{
		printf("sunxi sprite error : write item data failed\n");
...
uint Img_WriteOffset(void *buffer, uint offset,uint size)
{
	uint	      write_size = 0;
	fseek(img_file_hd, offset, SEEK_SET);
	write_size = fwrite(buffer, size, 1, img_file_hd);
	if (!write_size)
	{
		printf("sunxi sprite error : write  data failed\n");
