...
struct mixer *mixer;
static int m_spk = 0;
static int play_ok = 0;
static int headset_vol = 0x1d;
...
void set_to_speaker()
{
    tinymix_set_value_byname(mixer, "Audio linein in", 0);
    tinymix_set_value_byname(mixer, "speaker volume control", 28);
...
{
    tinymix_set_value_byname(mixer, "Audio linein in", 0);
    //tinymix_set_value_byname(mixer, "headphone volume control", headset_vol);
    tinymix_set_value_byname(mixer, "DAC left channel volume", 140);
    tinymix_set_value_byname(mixer, "DAC right channel volume", 140);
    tinymix_set_value_byname(mixer, "Speaker Function", 0);
}

void set_to_dmic()
{
    tinymix_set_value_byname(mixer, "Audio headsetmic voicerecord", 0);
}

void set_to_hmic()
{
    tinymix_set_value_byname(mixer, "Audio headsetmic voicerecord", 1);
}
...
#ifdef WAIT_HEADSET
void wait_headset(int require_mic)
{
    int fd;
    char jack_buf[16];
    int jack_state = 0;
    ...
    while (1) {
        ...
        jack_state = atoi(jack_buf);
        if (((1 == jack_state) && require_mic) ||
            ((jack_state > 0) && !require_mic)) {
            set_to_jack();
            db_msg("jack type#%d\n", atoi(jack_buf));
            break;
        }
...
int main(int argc, char **argv)
{
    ...
    if (script_fetch("mic", "headset_volume", &headset_vol, 1)) {
        headset_vol = 0x1d;
        db_warn("unknown headset_vol, use default:%d\n", headset_vol);
    }
...
static void *music_play(void *args)
{
    ...
    if (!pcm_is_ready(out_pcm)) {
        db_error("cannot open pcm_out driver: %s", pcm_get_error(out_pcm));
        goto out;
    }
    ...
    while (1) {
        while (!feof(fp)) {
            ...
            err = pcm_write(out_pcm, (void *)buf, BUF_LEN);
            if (err < 0) {
                db_warn("write error\n");
            }
        ...
        if (ret == 0) {
            ...
            if (!mic_passed) {
                ...
                if (mic1_used && mic2_used) {
                    if (mic1_passed && mic2_passed && play_ok) {
                        mic_passed = true;
                        db_msg("mic passed!\n");
                        SEND_CMD_PIPE_OK();
                    }
                } else if (mic1_used && !mic2_used) {
                    if(mic1_passed && play_ok){
                        mic_passed = true;
                        db_msg("mic passed!\n");
                        SEND_CMD_PIPE_OK();
                    }
                } else if (!mic1_used && mic2_used) {
                    if (mic2_passed && play_ok) {
                        mic_passed = true;
                        db_msg("mic passed!\n");
                        SEND_CMD_PIPE_OK();
                    }
