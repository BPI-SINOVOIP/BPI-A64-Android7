...
function init_defconf()
{
	...
	if [ x"$LICHEE_SECURE" != x"" ] ; then
		pattern="${LICHEE_CHIP}_${LICHEE_SECURE}${LICHEE_PLATFORM}_${LICHEE_BOARD}"
	fi
	defconf=`awk '$1=="'$pattern'" {print $2,$3}' ${LICHEE_BR_DIR}/scripts/mkrule`
	if [ -n "${defconf}" ] ; then
		...
		if [ x"$LICHEE_SECURE" != x"" ] ; then
			pattern="${LICHEE_CHIP}_${LICHEE_SECURE}${LICHEE_PLATFORM}"
		fi
		defconf=`awk '$1=="'$pattern'" {print $2,$3}' ${LICHEE_BR_DIR}/scripts/mkrule`
		if [ -n "${defconf}" ] ; then
			export LICHEE_BR_DEFCONF=`echo ${defconf} | awk '{print $1}'`
...
function select_platform()
{
    ...
    while true ; do
		...
		if [ "$LICHEE_PLATFORM" = "secureandroid" ] ; then
			LICHEE_PLATFORM="android"
			export LICHEE_SECURE="secure"
		elif [ "$LICHEE_PLATFORM" = "securedragonboard" ] ; then
			LICHEE_PLATFORM="dragonboard"
			export LICHEE_SECURE="secure"
		else
			export LICHEE_SECURE=""
		fi
                echo "export LICHEE_PLATFORM=$LICHEE_PLATFORM" >> .buildconfig
                echo "export LICHEE_SECURE=$LICHEE_SECURE" >> .buildconfig
                break
            fi
        fi
        printf "Invalid input ...\n"
