...
*loopback
*/
static int codec_set_loopback(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	int reg_val;
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	loopback_en = ucontrol->value.integer.value[0];
	...
	if (loopback_en) {
		...
		reg_val = (0x1<<LADCMIXMUTELOUTPUT)|(0x1<<RADCMIXMUTEROUTPUT);
		snd_soc_write(codec, ADC_SRC, reg_val);
...
static int codec_get_loopback(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = loopback_en;
	return 0;
}
...
static const struct snd_kcontrol_new sndvir_audio_controls[] = {
	...
	/* set loopback test */
	SOC_SINGLE_BOOL_EXT("Audio loopback", 0,
		codec_get_loopback, codec_set_loopback),
