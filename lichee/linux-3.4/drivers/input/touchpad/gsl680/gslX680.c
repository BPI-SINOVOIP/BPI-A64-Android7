...
static void max_x(int num){
   if(num == 0){
     max_size_x = min_size_x = key_x[num] ;
   }else{
     if(key_x[num] < min_size_x){
	 	min_size_x = key_x[num];
	 }else if(key_x[num] > max_size_x){
		max_size_x = key_x[num];
	 }
   }
}


static void max_y(int num){
   if(num == 0){
     max_size_y = min_size_y = key_y[num] ;
   }else{
     if(key_y[num] < min_size_y){
	 	min_size_y = key_y[num];
	 }else if(key_y[num] > max_size_y){
		max_size_y = key_y[num];
	 }
   }
}

static void old_wb_key(num){
...
if( key_count <= 512 )
			{
			if(((key_x[key_count  -1] - key_x[0]) > KEY_SIZE_X)&&((max_size_y - min_size_y)*2 < key_x[key_count  -1] - key_x[0]) )
				{
				    send_key = false;
					//keynum = 0;
				...
				else if(((key_x[0] - key_x[key_count-1]) > KEY_SIZE_X)&&((max_size_y - min_size_y)*2 < (key_x[0] - key_x[key_count-1])))
			 {
						send_key = false;
						//keynum = 0;
					...
					if(((key_y[key_count-1] - key_y[0]) > KEY_SIZE_Y)&&((max_size_x - min_size_x)/2 < (key_y[key_count-1] - key_y[0])))
					{
						send_key = false;
						//keynum = 0;
						...
						input_sync(ts->input);									
					}else if(((key_y[0] - key_y[key_count-1]) > KEY_SIZE_Y)&&((max_size_x - min_size_x)/2 < (key_y[0] - key_y[key_count-1])))
						{
							send_key = false;
							//keynum = 0;
						...
						if( (key_x[key_count -1] - key_x[0] < 50)  && (key_x[key_count -1] - key_x[0] >= -50) && (key_y[key_count -1] - key_y[0] < 50) &&(key_y[key_count -1] - key_y[0] >= -50) && (key_x[0] != 0) && (key_y[0] != 0))
