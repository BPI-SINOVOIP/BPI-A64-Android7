        ...
        for ( i = 0; i < POINT_MAX; i++ ) {
            ...
            if ( dc[0] > 0 ) {
                if ( ps[0][i].all == ps[1][i].all && temp != ps[0][i].all ) {
                    ps[0][i].all			= temp;
                    point_decimal[i].other.x	+= (short) ps[0][i].other.x - (short) ps[1][i].other.x;
                    point_decimal[i].other.y	+= (short) ps[0][i].other.y - (short) ps[1][i].other.y;
                    ps[0][i].other.x		= ps[1][i].other.x;
                    ps[0][i].other.y		= ps[1][i].other.y;
...
static void ResetMask( void )
{
    ...
    else if ( PointDistance( (gsl_POINT_TYPE *) (&reset_mask_max), pp[0] ) > ( ( (unsigned int) reset_mask_dis) & 0xffffff)
...
{
    if ( sqrt_able )
        return(GestureSqrt( (d1->other.x - d2->other.x) * (d1->other.x - d2->other.x) + (d1->other.y - d2->other.y) * (d1->other.y - d2->other.y) ) );
    else
        return( (d1->other.x - d2->other.x) * (d1->other.x - d2->other.x) + (d1->other.y - d2->other.y) * (d1->other.y - d2->other.y) );
}
...
static int GestureStretch( void )
{
    ...
    if ( x_max < x_min + 64 * 2 || y_max < y_min + 64 * 3 )
        return(FALSE);
    for ( i = 0; i < gesture_num; i++ ) {
        gesture_buf[i].other.x	= (gesture_buf[i].other.x - x_min) * GESTURE_SIZE_REFE / (x_max - x_min);
        gesture_buf[i].other.y	= (gesture_buf[i].other.y - y_min) * GESTURE_SIZE_REFE / (y_max - y_min);
    }
    return(TRUE);
}
