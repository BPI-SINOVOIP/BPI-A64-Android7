...
int ltr558_devinit(void)
{
	...
	*/
	...
	/* 0 ~ 500 */
	ltr558_i2c_write_reg(LTR558_PS_THRES_LOW_0, 0xff);
	ltr558_i2c_write_reg(LTR558_PS_THRES_LOW_1, 0x7);
	ltr558_i2c_write_reg(LTR558_PS_THRES_UP_0, 0x00);
	ltr558_i2c_write_reg(LTR558_PS_THRES_UP_1, 0x00);
...
{
	struct ltr_data *ltr = (struct ltr_data *)dev_id;
...
{
	struct ltr_data *ltr = container_of((struct work_struct *)work,
			struct ltr_data, irq_workqueue);
#if 1
	int als_ps_status;
	...
	*/
	...
	if (val == 0) {
		ltr558_i2c_write_reg(LTR558_PS_THRES_LOW_0, 0x80);
		ltr558_i2c_write_reg(LTR558_PS_THRES_LOW_1, 0x00);
		ltr558_i2c_write_reg(LTR558_PS_THRES_UP_0, 0xff);
		ltr558_i2c_write_reg(LTR558_PS_THRES_UP_1, 0xff);
		input_report_key(ltr->proximity_input_dev, KEY_WAKEUP, 1);
		input_sync(ltr->proximity_input_dev);
		msleep(100);
		input_report_key(ltr->proximity_input_dev, KEY_WAKEUP, 0);
		input_sync(ltr->proximity_input_dev);
	} else {
		ltr558_i2c_write_reg(LTR558_PS_THRES_LOW_0, 0x00);
		ltr558_i2c_write_reg(LTR558_PS_THRES_LOW_1, 0x00);
		ltr558_i2c_write_reg(LTR558_PS_THRES_UP_0, 0x80);
		ltr558_i2c_write_reg(LTR558_PS_THRES_UP_1, 0x00);
		input_report_key(ltr->proximity_input_dev, KEY_SLEEP, 1);
		input_sync(ltr->proximity_input_dev);
		msleep(100);
		input_report_key(ltr->proximity_input_dev, KEY_SLEEP, 0);
		input_sync(ltr->proximity_input_dev);
	}

	/*queue_delayed_work(ltr->wq, &ltr->ps_delay_work,
		*msecs_to_jiffies(ltr->ps_poll_delay));
			  ...
			  const struct i2c_device_id *id)
{
	...
	*/
	...
	if (ret) {
		dprintk(DEBUG_INIT, "%s: could not setup irq\n", __func__);
		goto err_sysfs_create_group_light;
	}
