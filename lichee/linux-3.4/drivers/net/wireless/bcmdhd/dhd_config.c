	...
	int def, bool down)
{
	...
	if (val >= def) {
		if (down) {
			bcmerror = dhd_wl_ioctl_cmd(dhd, WLC_DOWN, NULL, 0, TRUE, 0);
			if (bcmerror < 0)
				CONFIG_ERROR(("%s: WLC_DOWN setting failed %d\n", __FUNCTION__, bcmerror));
		}
		printf("%s: set %s %d %d\n", __FUNCTION__, name, cmd, val);
		bcmerror = dhd_wl_ioctl_cmd(dhd, cmd, &val, sizeof(val), TRUE, 0);
		if (bcmerror < 0)
	...
	int *val, int len, bool down)
{
	...
	bcmerror = dhd_wl_ioctl_cmd(dhd, cmd, val, len, TRUE, 0);
	if (bcmerror < 0)
		CONFIG_ERROR(("%s: %s setting failed %d\n", __FUNCTION__, name, bcmerror));
	...
	bool down)
{
	...
	if (val >= def) {
		...
		bcmerror = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
		if (bcmerror < 0)
