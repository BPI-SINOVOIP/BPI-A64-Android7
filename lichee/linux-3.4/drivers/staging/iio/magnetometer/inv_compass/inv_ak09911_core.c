...
{
	int ret, retry;
	uint8_t test_data[1];	/*only write a data address.*/
...
static int e_compass_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	int ret = 0;
	int result;
	unsigned char serial_data[1];
	pr_err("enter func %s. \n", __FUNCTION__);
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA))
		return -ENODEV;

	if (sensor_info.twi_id == adapter->nr) {
		pr_err("%s: addr= %x,adapter->nr : %d\n", __func__, client->addr, adapter->nr);
		ret = i2c_test(client);
		result = i2c_smbus_read_i2c_block_data(client, AK09911_REG_WIA, 1, serial_data);
		pr_err("AK09911_REG_WIA : %x\n", serial_data[0]);
		if (!ret) {
			printk("%s:I2C connection might be something wrong \n", __func__);
			return -ENODEV;
		} else {
			strlcpy(info->type, AKM_NAME, sizeof(AKM_NAME));
			return 0;
		}
...
static int ak09911_read_raw(struct iio_dev *indio_dev,
	...
	case IIO_CHAN_INFO_SCALE:
		...
		scale *= (1L << 15);
		*val = scale;
		printk("in the ak09911_read_raw\n");
			return IIO_VAL_INT;
		return -EINVAL;
	default:
		return -EINVAL;
...
void set_ak09911_enable(struct iio_dev *indio_dev, bool enable)
{
	struct inv_ak09911_state_s *st = iio_priv(indio_dev);
	int result = 0;
	pr_err("in set_ak09911_enable enable : %d,st->delay : %d\n", enable, st->delay);
	...
	if (enable) {
			...
			printk("in AK09911_CNTL_MODE_SNG_MEASURE \n");
	} else {
			pr_err("in the else \n");
...
static ssize_t ak09911_mag_value_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct inv_ak09911_state_s *st = iio_priv(indio_dev);
	...
	mutex_lock(&indio_dev->mlock);
	ak09911_read(st, st->compass_data);
	c[0] = st->compass_data[0];
	c[1] = st->compass_data[1];
	c[2] = st->compass_data[2];
	mutex_unlock(&indio_dev->mlock);
...
static DEVICE_ATTR(mag_value, S_IRUGO, ak09911_mag_value_show, NULL);
	...
	const struct i2c_device_id *id)
{
	...
	#ifdef ALL_WINNER
	st->plat_data = mpu_compass_data;
	#else
	st->plat_data =
		*(struct mpu_platform_data *)dev_get_platdata(&client->dev);
	#endif
	st->i2c_addr = client->addr;
	st->delay = AK09911_MIN_DELAY;
...
static int __init inv_ak09911_init(void)
{
	...
	inv_ak09911_driver.detect = e_compass_detect;
	printk("%s: success \n", __func__);
	#endif
	result = i2c_add_driver(&inv_ak09911_driver);
	if (result) {
		pr_err("%s failed\n", __func__);
		return result;
	}
