...
public class SoundSettings extends SettingsPreferenceFragment implements Indexable {
    ...
    // === 3D audio switch ===
    private void initUpdateAudio3dSurround() {
        mAudio3dSurround =
                (TwoStatePreference) getPreferenceScreen().findPreference(KEY_AUDIO_3D_SURROUND);
        ...
        if(!isDisplay3dSurround){
            getPreferenceScreen().removePreference(mAudio3dSurround);
        }
    }
	...
	private void initUpdateBootMusic() {
        mBootMusic = 
				(TwoStatePreference) getPreferenceScreen().findPreference(KEY_BOOT_MUSIC);
        if (mBootMusic == null) {
