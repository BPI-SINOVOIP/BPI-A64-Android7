...
static int set_normal_volume(struct codec_client *client, int path, int vol)
{
	...
	if (speaker_on){
		...
		set_headphone_volume(client->mixer_ctls->speaker_volume_control, vol);
	} else if ((headset_on || headphone_on)){
		ALOGV("****LINE:%d,FUNC:%s",__LINE__,__FUNCTION__);
		//mixer_ctl_set_value(client->mixer_ctls->master_playback_volume, 0, vol);
		set_headphone_volume(client->mixer_ctls->headphone_volume_control,vol);
	}
...
static int set_phone_path(struct codec_client *client, int path)
{
	...
	if (bt_on){
		...
		//mixer_ctl_set_value(client->mixer_ctls->audio_bt_clk_format_status, 0, 0);
	...
	mixer_ctl_set_value(client->mixer_ctls->audio_phone_out, 0,1);
		...
		if(earpiece_on && (no_earpiece == 0)){
			...
			mixer_ctl_set_value(client->mixer_ctls->audio_analog_main_mic, 0, 0);
			mixer_ctl_set_value(client->mixer_ctls->audio_analog_headsetmic, 0, 1);
			...
			//mixer_ctl_set_value(client->mixer_ctls->audio_bt_clk_format_status, 0, 1);
	...
	if (is_in_record){
		//client->record_ops->set_record_source(!bt_on);
		mixer_ctl_set_value(client->mixer_ctls->audio_analog_bb_capture_mic_switch, 0, 1);
	}
	#endif
#endif
	return 0;
