...
#if  USE_3D_SURROUND
	if (sur_enable(&sur)) {
		if (sur_prepare(&sur, out->devices, 1, out->common.cfg.rate,
			2, bytes / audio_stream_out_frame_size(stream))) {
			sur_process(&sur, (short*)buffer, bytes / audio_stream_out_frame_size(stream), 2);
		}
	}
#endif
...
#if 1
#if USE_3D_SURROUND
	sur_init(&sur, AUDIO_STREAM_DEFAULT_RATE, 2, AUDIO_STREAM_OUTPUT_BUFFER_SZ/4);
#endif
#endif
