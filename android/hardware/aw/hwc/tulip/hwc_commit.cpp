...
inline void hwc_init_config(struct disp_layer_config *layer_config, int count)
{
    int i = 0, j = 0;
...
{
    int i = 0, j = 0, lyr = 0;
    struct disp_layer_config *commit_layer = NULL;
...
void *commit_thread(void *priv)
{
    ...
    unsigned int stop_at_sync = 0, global_current_sync = 0;
	int unblank_count = 0;
    ...
    commit_data.hwc_layer_info[0] =
            (struct disp_layer_config *)calloc(16, sizeof(struct disp_layer_config));
    commit_data.hwc_layer_info[1] =
            (struct disp_layer_config *)calloc(8, sizeof(struct disp_layer_config));
	...
	while(1)
	{
        ...
        if(!hwc_list_empty(CommitHead) || (Globctx->has_cursor && (Globctx->cursor_sync >= current_sync_count)))
        {
            ...
            if(!hwc_list_empty(CommitHead))
            {
                ...
                continues_stop = 0;
                global_current_sync = DisplayData->sync_count;
                /* debug pause */
                pause = hwc_debug_pause(Globctx, DisplayData->sync_count, pause);
                /* deal the tr video and the tr UI */
                while( j < NUMBEROFDISPLAY && !pause)
                {
                    if(DisplayData->has_tr[j] != 0
                        && !commit_data.force_flip[j])
                    {
                        ...
                        int tmp_tr_cnt = 0;
                        for(i = 0, tmp_tr_cnt = 0;
                                (i < DisplayData->layer_num_inused[j])
                                    && (tmp_tr_cnt < DisplayData->has_tr[j])
                                    && (commit_data.force_flip[j] == 0);
                                 i++)
                        {
                            if(DisplayData->hwc_layer_info[j][i].tr == 0)
                            {
                                continue;
                            }
                            if(DisplayData->cursor_in_disp[j] == i)
                            {
                                continue;
                            }
                            tmp_tr_cnt++;
                            has_tr++;
                            tr_fence_fd = DisplayData->hwc_layer_info[j][i].aquirefencefd;
                            need_sync = DisplayData->hwc_layer_info[j][i].needsync;
                            share_fd = DisplayData->hwc_layer_info[j][i].share_fd;
                            if(tr_fence_fd >= 0)
                            {
                                ret = sync_wait(tr_fence_fd, 3000);
                                if(ret < 0)
                                {
                                    ALOGE("wait tr layer fence timeout(%d)", ret);
                            ...
                            if(need_sync)
                            {
                                ...
                                DisplayData->hwc_layer_info[j][i].needsync = 0;
                            }
                            /* debug dump layer to /data/dump% */
                            hwc_debug_dump_layer(Globctx, &DisplayData->hwc_layer_info[j][i], j, DisplayData->sync_count);

                            if(!hwc_rotate_layer_tr(DisplayData, &commit_data, j, i, has_tr+1))
                            {
...
deal_fence:
                ...
                while( j < NUMBEROFDISPLAY && !pause)
                {
                    for(i = 0; i < DisplayData->layer_num_inused[j]; i++)
                    {
                        ...
                        /* debug dump layer to /data/dump_% */
                        hwc_debug_dump_layer(Globctx, &DisplayData->hwc_layer_info[j][i], j, DisplayData->sync_count);
                        if(DisplayData->hwc_layer_info[j][i].aquirefencefd >= 0)
                        {
                ...
                /* debug close layer */
                hwc_debug_close_layer(Globctx, &commit_data);
                ...
                /* ion ref, avoid release the buffer by dma_buffer */
                hwc_manage_ref_cache(!(commit_data.force_flip[0] && commit_data.force_flip[1]), DisplayData);
                /* updata the limit for memory */
                if(!Globctx->hot_plug)
                {
                    hwc_updata_limit(Globctx, DisplayData, local_mem, stop_at_sync);
                }else{
                    if(DisplayData->layer_num_inused[0] != 0
                        && DisplayData->layer_num_inused[1] != 0)
                        Globctx->hot_plug = 0;
                    stop_at_sync = global_current_sync;
                    hwc_down_limit(Globctx, local_mem, 0);
                }
                /* Commit List Cache manage */
            ...
            /* down the limit and the stop the hwc */
            if(continues_stop == 60)
            {
                stop_at_sync = global_current_sync;
                hwc_down_limit(Globctx, local_mem, 1);
            }
