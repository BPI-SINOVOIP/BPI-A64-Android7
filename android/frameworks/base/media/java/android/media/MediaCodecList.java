...
final public class MediaCodecList {
	...
	private static final String CTS_MEDIA_PACKAGE_NAME = "com.android.cts.media";
	private static final String CTS_MEDIA_CLASS_NAME = "android.media.cts.EncodeVirtualDisplayWithCompositionTest";
	...
	if(ActivityThread.currentProcessName().equals(CTS_MEDIA_PACKAGE_NAME) 
		&& (SystemProperties.getBoolean("ro.config.512m", false)
		|| SystemProperties.getBoolean("ro.config.media", false))){
	    Exception e1 = new Exception();
	    StackTraceElement[] st = e1.getStackTrace();

	    for(int i=0;i<st.length;i++){
		if (st[i].toString().startsWith(CTS_MEDIA_CLASS_NAME)){
		    Log.w("MediaCodeList","Calling android.media.cts.EncodeVirtualDisplayWithCompositionTest!");
		    int width = format.getInteger("width");
		    int height = format.getInteger("height");
		    if(width >= 1280 && height >= 720){
			Log.w("MediaCodeList","Current encoding and decoding on low memory device does not support w:" + width + " h:" + height);
			return null;
		    }
	        }
	    }
