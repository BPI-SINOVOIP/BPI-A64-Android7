...
final class ProcessList {
    ...
    static final int MIN_CACHED_APPS = (SystemProperties.getBoolean("ro.config.512m", false))? 0: 2;
    ...
    // process limit.
    static final int MAX_CACHED_APPS = (SystemProperties.getBoolean("ro.config.512m", false))? 0: 32;
    ...
    private void updateOomLevels(int displayWidth, int displayHeight, boolean write) {
        ...
        if (write) {
            ...
            //customized forground oom value
            if (SystemProperties.getBoolean("ro.config.512m", false)) {
                mOomMinFree[0] = 8192;
            }
            for (int i=0; i<mOomAdj.length; i++) {
