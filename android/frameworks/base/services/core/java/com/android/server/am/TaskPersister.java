...
public class TaskPersister {
    ...
    private static final boolean DISABLE_TASKPERSISTER = (SystemProperties.getBoolean("ro.config.512m", false));
    ...
    void startPersisting() {
        if (DISABLE_TASKPERSISTER) return;
    ...
    Bitmap getTaskDescriptionIcon(String filePath) {
        if (DISABLE_TASKPERSISTER) return null;
    ...
    List<TaskRecord> restoreTasksForUserLocked(final int userId) {
		if (DISABLE_TASKPERSISTER) return new ArrayList<TaskRecord>();
