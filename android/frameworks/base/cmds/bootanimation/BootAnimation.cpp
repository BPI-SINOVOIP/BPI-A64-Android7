...
static const int MAX_DECODE_THREADS = 8;
static const int MAX_DECODE_CACHE = 8;
#endif
BootAnimation::BootAnimation(char *animation) : Thread(false), mClockEnabled(true), mTimeIsAccurate(false),
        mTimeCheckThread(NULL) {
    mCustomAnimationFile = animation;
...
status_t BootAnimation::readyToRun() {
    ...
    if (mCustomAnimationFile != NULL && (access(mCustomAnimationFile, R_OK) == 0)) {
        mZipFileName = mCustomAnimationFile;
    }
    else if (encryptedAnimation && (access(SYSTEM_ENCRYPTED_BOOTANIMATION_FILE, R_OK) == 0)) {
...
bool BootAnimation::playAnimation(const Animation& animation)
{
    ...
    for (size_t i=0 ; i<pcount ; i++) {
        ...
        for (int r=0 ; !part.count || r<part.count ; r++) {
            ...
            if (r == 0 && mAudioPlayer != NULL && part.audioFile&& playSoundsAllowed()) {
