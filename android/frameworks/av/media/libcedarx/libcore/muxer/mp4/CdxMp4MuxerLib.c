...
static cdx_int32 writeBufferStream(ByteIOContext *s, cdx_uint8 *buf, cdx_int32 size)
{
#if FS_WRITER
...
{
    writeBufferStream(s, (cdx_uint8*)(&b), 1);
}
...
static void writeBe32Stream(ByteIOContext *s, cdx_uint32 val)
{
    ...
    val= (val>>16) | (val<<16);
    writeBufferStream(s, (cdx_uint8*)&val, 4);
}
...
int avccWriteSpsPps(ByteIOContext *pb, cdx_uint8 *data, int len, MOVTrack *track)
{
    if (len > 6)
    {
        ...
        if (MOV_AV_RB32(data) == 0x00000001)
        {
            ...
            writeBufferStream(pb, (cdx_uint8*)sps, sps_size);
            writeByteStream(pb, 1); /* number of pps */
            writeBe16Stream(pb, pps_size);
            writeBufferStream(pb, (cdx_uint8*)pps, pps_size);
            free(start);
        }
        else
        {
            writeBufferStream(pb, (cdx_uint8*)data, len);
        }
    }
    return 0;
}
...
static cdx_int32 mdiaWriteHdlrTag(ByteIOContext *pb, MOVTrack *track)
{
    ...
    writeBe32Stream(pb, 0); /* Version & flags */
    writeBufferStream(pb, (cdx_uint8*)hdlr, 4); /* handler */
    writeTagStream(pb, hdlr_type); /* handler type */
    writeBe32Stream(pb ,0); /* reserved */
    writeBe32Stream(pb ,0); /* reserved */
    writeBe32Stream(pb ,0); /* reserved */
    writeByteStream(pb, strlen((const char *)descr)); /* string counter */
    writeBufferStream(pb, (cdx_uint8*)descr, strlen((const char *)descr)); /* handler description */
...
static void writeLatitude(ByteIOContext *pb, int degreex10000)
{
    ...
    writeBufferStream(pb, (cdx_uint8 *)str, 8);
}
...
static void writeLongitude(ByteIOContext *pb, int degreex10000)
{
    ...
    // Do not write the null terminator
    writeBufferStream(pb, (cdx_uint8 *)str, 9);
}
...
cdx_int32 Mp4WritePacket(Mp4MuxContext *s, CdxMuxerPacketT *pkt)
{
    ...
    cdx_uint8  *pkt_buf = (cdx_uint8*)(pkt->buf);
    ...
    {  /* gushiming compressed source */
        ...
        writeBufferStream(pb, (cdx_uint8*)tmp_mjpeg_trailer, 2);
    }
